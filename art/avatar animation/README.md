...
## Laugh animation (tap interaction)
`avatar/frida-laugh.html` is a **self-contained reference implementation**: tap/click Frida → she laughs for ~1s. Mechanic: toggle a `laughing` class on the wrapper for 1050ms (force reflow before re-adding so rapid taps restart it). While laughing: the `.normal-face` SVG group hides, `.laugh-face` shows (closed happy eyes + open mouth with tongue), the figure plays `fridaLaugh` (squash-and-stretch bounce, transform-origin bottom-center, 1s ease) and three "ha!" bubbles play `fridaHa` (rise + fade, staggered 0s / 0.18s / 0.36s). Keyframes are in the file — copy them verbatim. Honors `prefers-reduced-motion` (no motion, bubbles just appear). Keyboard-accessible (`role="button"`, Enter/Space). Recommended uses: tapping the mascot anywhere, correct-answer feedback.

## Favicons & app icons
`avatar/favicon/` contains favicon.svg + 16/32/48px transparent PNGs, apple-touch-icon (180px) and PWA icons (192/512px, teal `#2FA79B` background). Its README.md has the exact `<head>` tags and `site.webmanifest` to paste in.
...
- `avatar/frida-laugh.html` — tap-to-laugh animation reference (standalone, open directly)
- `avatar/favicon/` — favicon + app icon set with install instructions
